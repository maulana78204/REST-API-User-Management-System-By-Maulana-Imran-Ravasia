...
import com.example.usermanagement.service.AppUserService;
import com.example.usermanagement.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link com.example.usermanagement.domain.AppUser}.
 */
@RestController
...
public class AppUserResource {
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<AppUser>> getAllAppUsers(@org.springdoc.core.annotations.ParameterObject Pageable pageable, @RequestParam(name = "filter", required = false) String filter) {
        if ("userprofile-is-null".equals(filter)) {
            LOG.debug("REST request to get all AppUsers where userProfile is null");
            return new ResponseEntity<>(appUserService.findAllWhereUserProfileIsNull(), HttpStatus.OK);
        }
